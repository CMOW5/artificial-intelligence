...
def minimax(board):
    ...
    if player(board) == X:
        ...
        scores = [min_value(result(board, action)) for action in legal_moves]
        ...
        print("chosenIndex =", chosen_index)
        print("chosen move =", legal_moves[chosen_index])
        print("tracker = " + str(Tracker.count))
        return legal_moves[chosen_index]

    if player(board) == O:
        legal_moves = actions(board)
        scores = [max_value(result(board, action)) for action in legal_moves]
...
def max_value(board_state):
    ...
    for action in actions(board_state):
        v = max(v, min_value(result(board_state, action)))

    return v


def min_value(board_state):
    Tracker().track()
    ...
    for action in actions(board_state):
        v = min(v, max_value(result(board_state, action)))
